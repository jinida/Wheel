...
{
    /// <summary>
    /// Evaluate image table component - displays evaluation images with predictions
    /// Shows only Validation and Test set images
    /// </summary>
    public partial class EvaluateImageTable : ComponentBase, IAsyncDisposable
    {
        [Inject]
        ...
        /// <summary>
        /// Project type to determine label display
        /// </summary>
        [Parameter]
        ...
        /// </summary>
        [Parameter]
        public Dictionary<int, PredictionDto> Predictions { get; set; } = new();
        ...
        /// </summary>
        private string GetLabelFromImage(ImageDto image)
        {
        ...
        /// <summary>
        /// Gets prediction display text for an image
        /// </summary>
        private string GetMockPrediction(ImageDto image)
        {
            if (!Predictions.ContainsKey(image.Id) || !Predictions[image.Id].Annotations.Any())
                return "-";

            var prediction = Predictions[image.Id];

            return ProjectType switch
            {
                0 or 3 => // Classification/Anomaly: show class name
                    prediction.Annotations.FirstOrDefault()?.ClassName ?? "-",
                1 or 2 => // Detection/Segmentation: show count per class
                    GetPredictionCountByClass(prediction.Annotations),
                _ => "-"
            };
        }

        /// <summary>
        /// Gets prediction count grouped by class name
        /// Format: "2 Car, 1 Person, 3 Tree"
        /// </summary>
        private string GetPredictionCountByClass(List<PredictionAnnotationDto> annotations)
        {
            var grouped = annotations
        ...
        protected override async Task OnAfterRenderAsync(bool firstRender)
        {
            if (firstRender || !_isInitialized)
            {
                ...
                try
                {
                    await JSRuntime.InvokeVoidAsync("wheelApp.initializeColumnResizing", "evaluate-grid-container", true);
                }
                catch (Exception ex)
                {
        ...
        public async ValueTask DisposeAsync()
        {
            try
            {
                ...
                await JSRuntime.InvokeVoidAsync("wheelApp.cleanupColumnResizing", "evaluate-grid-container");
            }
            catch (Exception ex)
            {
